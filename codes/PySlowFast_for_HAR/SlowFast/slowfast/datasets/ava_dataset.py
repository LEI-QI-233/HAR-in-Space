...
class Ava(torch.utils.data.Dataset):
    ...
    def _load_data(self, cfg):
        ...
        # 2. 读取框 + 标签，并且拿到 id 映射表（已经在 ava_helper 里完成标签压紧）
        boxes_and_labels_dict, self.id_map, self.inv_map = ava_helper.load_boxes_and_labels(
            cfg, mode=self._split
        )
        self._num_classes = len(self.id_map)

        # 3. 把 dict 映射成与 _image_paths 对齐的 list（保持原有逻辑）
        assert len(boxes_and_labels_dict) == len(self._image_paths)
        boxes_and_labels = [
            boxes_and_labels_dict[self._video_idx_to_name[i]]
            for i in range(len(self._image_paths))
        ]
    ...
    def __getitem__(self, idx):
        ...
        # ------------------------------------------------------------------
        # 4. 构造 one-hot 标签矩阵
        #    现在 label 列表里的数字已经是 0 ~ self._num_classes-1，
        #    不再是 AVA 原始 id，也无需再做 (label-1) 的偏移。
        # ------------------------------------------------------------------
        label_arrs = np.zeros((len(labels), self._num_classes), dtype=np.int32)

        for i, box_labels in enumerate(labels):
            for new_label in box_labels:          # 已经是连续索引
                if 0 <= new_label < self._num_classes:
                    label_arrs[i][new_label] = 1
                else:
                    # 出现越界说明上游数据异常，可留 log 或直接跳过
                    logger.warning(
                        "Label %d out of range (0-%d)", new_label, self._num_classes - 1
                    )

        # ------------------------------------------------------------------
        # 5. 打包多路径输出并组装 metadata
        # ------------------------------------------------------------------
        imgs = utils.pack_pathway_output(self.cfg, imgs)
        metadata = [[video_idx, sec]] * len(boxes)
