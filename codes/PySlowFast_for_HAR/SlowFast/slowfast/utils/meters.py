...
class AVAMeter:
    ...
    def __init__(self, overall_iters, cfg, mode):
        ...
        # ---------- 新增 ----------
        self.f1_macro   = 0.0   # macro-F1
        self.auroc_macro = 0.0  # macro-AUROC
        self.recall_macro = 0.0 # macro-recall
        self.ap_per_class = {} # map per class
    ...
    def finalize_metrics(self, log=True):
        ...
        if log:
            ...
            logging.log_json_stats(stats, self.output_dir)
            self.save_data_to_csv_file(None)
    ...
    def log_epoch_stats(self, cur_epoch):
        ...
        if self.mode in ["val", "test"]:
            ...
            self.save_data_to_csv_file(cur_epoch)

    def save_data_to_csv_file(self,cur_epoch):
        # ----save ap per class to csv file into detailed_csv subfolder----
        # 子目录名
        detailed_dir = os.path.join(self.output_dir, "detailed_csv")
        # 创建子目录
        pathmgr.mkdirs(detailed_dir)
        
        nametail = self.mode if cur_epoch is None else f"{self.mode}_{cur_epoch + 1}"
            
        
        # 构造 CSV 文件路径
        per_class_csv = os.path.join(
            detailed_dir, f"ap_per_class_{nametail}.csv"
        )
        # 写入 CSV
        with pathmgr.open(per_class_csv, "w") as wf:
            writer = csv.writer(wf)
            # 表头，一律写三列
            writer.writerow(["action_id", "ap50", "action_name"])
            for cid, info in self.ap_per_class.items():
                ap50 = info.get("ap50", 0.0)
                name = info.get("name", "")
                # 如果 name 是自动生成的默认名字（class_<cid>），当成“无真实名字”
                default_name = f"class_{cid}"
                if name and name != default_name:
                    # 真正有名字，写三列
                    writer.writerow([cid, f"{ap50:.5f}", name])
                else:
                    # 无真实名字，只写前两列
                    writer.writerow([cid, f"{ap50:.5f}"])
                    
        det_file = os.path.join(
            detailed_dir, f"detections_{nametail}.csv"
        )
        write_results(self.detections, det_file)

        # 打日志
        logger.info(
            "Saved CSVs:\n  per-class AP: %s\n  detections: %s",
            per_class_csv, det_file
        )
