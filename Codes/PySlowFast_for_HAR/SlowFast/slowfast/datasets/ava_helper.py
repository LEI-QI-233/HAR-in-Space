...
def load_boxes_and_labels(cfg, mode):
    """
    Loading boxes and labels from csv files,
    并把原始 AVA id 映射成连续 0..N-1。

    返回:
        all_boxes : 标签已映射为连续编号
        id_map    : {orig_id -> new_idx}
        inv_map   : {new_idx  -> orig_id}
    """
    gt_lists = cfg.AVA.TRAIN_GT_BOX_LISTS if mode == "train" else []
    pred_lists = (
        cfg.AVA.TRAIN_PREDICT_BOX_LISTS
    ...
    # -------- 构造映射并把标签压紧 --------
    # 如果 label_map_file 不是绝对路径，就默认放在 ANNOTATION_DIR 里
    label_map_path = cfg.AVA.LABEL_MAP_FILE
    if not os.path.isabs(label_map_path):
        label_map_path = os.path.join(cfg.AVA.ANNOTATION_DIR, label_map_path)

    id_map, inv_map = build_id_map(label_map_path)


    for video in all_boxes:
        for frame in all_boxes[video]:
            for box, label_list in all_boxes[video][frame]:
                new_labels = [id_map[l] for l in label_list if l != -1]
                label_list[:] = new_labels   # 原地替换

    # -----------------------------------------

    return all_boxes, id_map, inv_map
...
def get_keyframe_data(boxes_and_labels):
    ...
    NOTE: This version assumes your CSV 中直接输入的是帧编号，而不是秒。
    """
    ...
    for video_idx in range(len(boxes_and_labels)):
        ...
        for frame_num in boxes_and_labels[video_idx].keys():
            if len(boxes_and_labels[video_idx][frame_num]) > 0:
                keyframe_indices.append((video_idx, sec_idx, frame_num, int(frame_num)))
                keyframe_boxes_and_labels[video_idx].append(
                    boxes_and_labels[video_idx][frame_num]
                )
                sec_idx += 1
                count += 1
...
def parse_bboxes_file(ann_filenames, ann_is_gt_box, detect_thresh, boxes_sample_rate=1):
    ...
    Args:
        ...
        ann_is_gt_box (list of bools): list of bools indicating whether file is GT.
        detect_thresh (float): score threshold for predicted boxes.
        boxes_sample_rate (int): use 1 every N frames.
    """
    all_boxes = {}
    count = 0
    unique_box_count = 0
    for filename, is_gt_box in zip(ann_filenames, ann_is_gt_box):
        with pathmgr.open(filename, "r") as f:
            for line in f:
                ...
                video_name, frame_num = row[0], int(row[1])
                if frame_num % boxes_sample_rate != 0:
                    continue
                ...
                if frame_num not in all_boxes[video_name]:
                    all_boxes[video_name][frame_num] = {}

                if box_key not in all_boxes[video_name][frame_num]:
                    all_boxes[video_name][frame_num][box_key] = [box, []]
                    unique_box_count += 1

                all_boxes[video_name][frame_num][box_key][1].append(label)
                if label != -1:
                    count += 1

    for video_name in all_boxes.keys():
        for frame_num in all_boxes[video_name].keys():
            all_boxes[video_name][frame_num] = list(
                all_boxes[video_name][frame_num].values()
            )

    return all_boxes, count, unique_box_count

def build_id_map(label_map_file):
    """
    读取 pbtxt 文件，生成
        id_map : {原始 AVA id → 连续索引 0…N-1}
        inv_map: {连续索引 → 原始 AVA id}
    """
    orig_ids = []
    with open(label_map_file, "r") as f:
        for line in f:
            ...
            if line.startswith("id:"):
                orig_ids.append(int(line.split(":")[1]))
    orig_ids = sorted(orig_ids)
    id_map  = {oid: idx for idx, oid in enumerate(orig_ids)}
    inv_map = {idx: oid for oid, idx in id_map.items()}
