...
class X3D(nn.Module):
    ...
    def _construct_network(self, cfg):
        ...
        self.s1 = stem_helper.VideoModelStem(
            dim_in=cfg.DATA.INPUT_CHANNEL_NUM,
            dim_out=[round_width(self.dim_c1, w_mul)],
            kernel=[temp_kernel[0][0] + [3, 3]],
            stride=[[1, 2, 2]],
            padding=[[temp_kernel[0][0][0] // 2, 1, 1]],
            norm_module=self.norm_module,
        ...
        # stage2-5
        dim_in = round_width(self.dim_c1, w_mul)
        for stage, block in enumerate(self.block_basis):
            repeats = self._round_repeats(block[0], d_mul)
            dim_out = round_width(block[1], w_mul)
            dim_inner_stage = int(cfg.X3D.BOTTLENECK_FACTOR * dim_out)
            prefix = f"s{stage+2}"
            s = resnet_helper.ResStage(
                dim_in=[dim_in],
                dim_out=[dim_out],
                dim_inner=[dim_inner_stage],
                temp_kernel_sizes=temp_kernel[1],
                stride=[block[2]],
                num_blocks=[repeats],
                num_groups=[dim_inner_stage] if cfg.X3D.CHANNELWISE_3x3x3 else [num_groups],
                num_block_temp_kernel=[repeats],
                nonlocal_inds=cfg.NONLOCAL.LOCATION[0],
                nonlocal_group=cfg.NONLOCAL.GROUP[0],
                nonlocal_pool=cfg.NONLOCAL.POOL[0],
                instantiation=cfg.NONLOCAL.INSTANTIATION,
                ...
                norm_module=self.norm_module,
                dilation=cfg.RESNET.SPATIAL_DILATIONS[stage],
                drop_connect_rate=cfg.MODEL.DROPCONNECT_RATE * (stage + 2) / (len(self.block_basis) + 1),
            )
            setattr(self, prefix, s)
            dim_in = dim_out

        # 根据是否启用检测模式，选择不同的 head
        if self.enable_detection:
            # 使用 ResNetRoIHead 进行时空 ROI pooling
            pool_size = _POOL1[cfg.MODEL.ARCH]
            # pool_size[0][0] 对应 temporal downsample factor，x3d 是 1
            self.head = head_helper.ResNetRoIHead(
                dim_in=[dim_in],
                num_classes=cfg.MODEL.NUM_CLASSES,
                pool_size=[[cfg.DATA.NUM_FRAMES // pool_size[0][0], 1, 1]],
    ...
    def forward(self, x, bboxes=None):
        """
        x: list of Tensor (len=1)
        bboxes: 检测时传入的时空 bbox
        """
        # backbone 多路径输入（即使只有一路，也是 list）
        # s1: VideoModelStem 接受并返回 list
        x = self.s1(x)
        x = self.s2(x)
        x = self.s3(x)
        x = self.s4(x)
        ...
        if self.enable_detection:
            # 检测模式：ResNetRoIHead 接受 list-of-Tensor
            return self.head(x, bboxes)
        else:
            # 分类模式：X3DHead 接受单个 Tensor，需要解包
...
class MViT(nn.Module):
    ...
    def forward(self, x, bboxes=None, return_attn=False):
        ...
        assert len(bcthw) == 5 and T == self.T, bcthw
        B, N, C = x.shape
